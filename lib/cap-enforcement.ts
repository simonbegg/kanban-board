...
export async function checkBoardCap(userId: string): Promise<CapCheckResult> {
  ...
  return {
    ...
    reason: canCreate
      ? undefined
      : `Board limit reached (${currentBoards}/${entitlements.board_cap}). Upgrade to Pro for more boards.`,
...
export async function checkTaskCap(
  ...
  return {
    ...
    reason: canCreate
      ? undefined
      : `Task limit reached for this board (${currentTasks}/${entitlements.active_cap_per_board}). Archive some tasks to make room.`,
...
export async function getUserUsageStats(
  ...
  // If no entitlements exist, call the init API to create default Free plan
  if (!entitlements) {
    console.log("No entitlements found for user, initializing via API...");
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("Init API failed:", {
          status: response.status,
          statusText: response.statusText,
          body: errorText,
        });
        throw new Error(
          `Failed to initialize user entitlements: ${response.status} ${errorText}`
        );
      }
      ...
      // Retry fetching entitlements
      const { data: newEntitlements, error: retryError } = await supabase
        .from("entitlements")
        .select("plan, board_cap, active_cap_per_board, archived_cap_per_user")
        .eq("user_id", userId)
        .maybeSingle();
      ...
      if (retryError) {
        console.error("Failed to fetch entitlements after init:", retryError);
        return null;
      }

      if (!newEntitlements) {
        console.error("Entitlements still missing after init success");
        return null;
      }
...
async function calculateUsageStats(
  ...
  const { data: tasks } = await supabase
    ...
    .in(
      "board_id",
      (
        await supabase.from("boards").select("id").eq("user_id", userId)
      ).data?.map((b) => b.id) || []
    );
